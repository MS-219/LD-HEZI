...
func getDeviceSN() string {
	...
	if data, err := os.ReadFile(path); err == nil {
		...
		if sn != "" {
			if strings.HasPrefix(sn, "JX-") || strings.HasPrefix(sn, "LD-") {
				sn = "CD-" + sn[3:]
	...
	if len(components) > 0 {
		...
		finalSN = "CD-" + strings.ToUpper(hex.EncodeToString(hash[:])[:12])
	} else {
		// 万一啥都读不到，用时间戳兜底
		finalSN = "CD-R-" + time.Now().Format("0102150405")
	}
...
func generateBindCode(sn string) string {
	...
	return "CD" + strings.ToUpper(hexStr[:6])
}
